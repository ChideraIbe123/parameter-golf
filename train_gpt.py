...
def _build_cu_seqlens_from_batch(tokens,bos_token_id,device,bucket=64):
	bsz,seqlen=tokens.shape;flat=tokens.reshape(-1);bos_mask=(flat==bos_token_id);bos_pos=torch.nonzero(bos_mask,as_tuple=False).squeeze(-1).tolist();total=bsz*seqlen;starts=[]
	if not bos_pos or bos_pos[0]!=0:starts.append(0)
	last_doc_boundary=-1
	for p in bos_pos:
		if p!=last_doc_boundary:starts.append(p);last_doc_boundary=p
	boundaries=starts+[total];padded_len=((len(boundaries)+bucket-1)//bucket)*bucket;cu=torch.full((padded_len,),total,dtype=torch.int32,device=device);cu[:len(boundaries)]=torch.tensor(boundaries,dtype=torch.int32,device=device);seg_ends=starts[1:]+[total];max_seqlen=max((e-s for s,e in zip(starts,seg_ends)),default=total);return cu,int(max_seqlen)
...
class CausalSelfAttention(nn.Module):
	...
	def forward(self,x,cu_seqlens=None,max_seqlen=0):
		...
		if cu_seqlens is not None:y=flash_attn_varlen_func(q.reshape(bsz*seqlen,self.num_heads,self.head_dim),k.reshape(bsz*seqlen,self.num_kv_heads,self.head_dim),v.reshape(bsz*seqlen,self.num_kv_heads,self.head_dim),cu_seqlens_q=cu_seqlens,cu_seqlens_k=cu_seqlens,max_seqlen_q=max_seqlen,max_seqlen_k=max_seqlen,causal=True).reshape(bsz,seqlen,self.num_heads,self.head_dim)
...
class Block(nn.Module):
	...
	def forward(self,x,x0,cu_seqlens=None,max_seqlen=0):
		mix=self.resid_mix.to(dtype=x.dtype);x_in=mix[0][None,None,:]*x+mix[1][None,None,:]*x0;attn_out=self.attn(self.attn_norm(x_in)*self.ln_scale_factor,cu_seqlens=cu_seqlens,max_seqlen=max_seqlen)
...
class GPT(nn.Module):
	...
	def forward_logits(self,input_ids,cu_seqlens=None,max_seqlen=0):
		...
		for i in enc_iter:x=self.blocks[i](x,x0,cu_seqlens=cu_seqlens,max_seqlen=max_seqlen);skips.append(x)
	...
	def forward(self,input_ids,target_ids,cu_seqlens=None,max_seqlen=0):logits=self.forward_logits(input_ids,cu_seqlens=cu_seqlens,max_seqlen=max_seqlen);return F.cross_entropy(logits.reshape(-1,logits.size(-1)).float(),target_ids.reshape(-1),reduction='mean')
...
def gptq_mixed_quantize(state_dict,hessians,h):
	...
	for cat in sorted(categories):log("  "+cat+": "+", ".join(sorted(categories[cat])))
	return result,meta
...
def eval_val_ttt(h,device,val_data,base_model,batch_seqs=32):
	...
	log(f"ttt:start chunks={num_chunks} ttt_lr={h.ttt_lr} ttt_epochs={h.ttt_epochs} e2e_mode={h.ttt_e2e_mode}");compiled_logits=torch.compile(base_model.forward_logits,dynamic=False,fullgraph=True);loss_sum=torch.zeros((),device=device,dtype=torch.float64);token_count=torch.zeros((),device=device,dtype=torch.float64);byte_count=torch.zeros((),device=device,dtype=torch.float64)
	if h.ttt_e2e_mode:
		ttt_start_block=max(0,int(h.num_layers*(1.-h.ttt_e2e_last_frac)));ttt_params=[]
		for(n,p) in base_model.named_parameters():
			in_block=n.startswith('blocks.');is_mlp='.mlp.' in n;idx=-1
			if in_block:
				try:idx=int(n.split('.')[1])
				except ValueError:idx=-1
			p.requires_grad_(is_mlp and idx>=ttt_start_block);(ttt_params.append(p) if p.requires_grad else None)
		log(f"ttt:e2e mlp-only blocks>={ttt_start_block} n_params={sum(p.numel() for p in ttt_params)}")
	else:ttt_params=[p for p in base_model.parameters()];[p.requires_grad_(True) for p in ttt_params]
...
def train_model(h,device,val_data):
	base_model=GPT(h).to(device).bfloat16();restore_fp32_params(base_model);compiled_model=torch.compile(base_model,dynamic=False,fullgraph=not h.varlen_enabled)
	...
	def step_fn(step,lr_scale):
		...
		for micro_step in range(h.grad_accum_steps):
			...
			if h.varlen_enabled:cu_seqlens,max_seqlen=_build_cu_seqlens_from_batch(x,h.bos_token_id,device)
			with torch.autocast(device_type='cuda',dtype=torch.bfloat16,enabled=True):loss=model(x,y,cu_seqlens=cu_seqlens,max_seqlen=max_seqlen)
