...
class Hyperparameters:
    ...
    xsa_last_n = int(os.environ.get("XSA_LAST_N", 11))  # XSA on all layers (0 = disabled)
...
def quantize_int6_gptq(weight: Tensor, hessian: Tensor | None = None,
    ...
    """Full GPTQ with Cholesky error compensation. Falls back to percentile search without hessian."""
    t32 = weight.float()
    ...
    for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
        ...
        for i1 in range(0, cols, block_size):
            ...
            W1 = W_work[:, i1:i2].clone(); Err1 = torch.zeros(rows, i2 - i1)
...
def collect_hessians(model, loader, args, device, gas, num_batches=128):
    """Collect H = X^T X per CastedLinear for GPTQ calibration."""
    H = {}; hooks = []
    for name, m in model.named_modules():
        if isinstance(m, CastedLinear):
            pn = name + ".weight"; c = m.weight.shape[1]
            H[pn] = torch.zeros(c, c, dtype=torch.float32, device='cpu')
            def mk(p, nc):
                def fn(mod, inp, out):
                    x = inp[0].detach().float().reshape(-1, nc)
                    H[p] += (x.T @ x).cpu()
                return fn
            hooks.append(m.register_forward_hook(mk(pn, c)))
    model.eval()
    with torch.inference_mode(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        for _ in range(num_batches):
            x, y = loader.next_batch(args.train_batch_tokens, args.train_seq_len, gas)
            model(x, y)
    for h in hooks: h.remove()
    for k in H:
        H[k] /= num_batches
        d = 0.01 * torch.diag(H[k]).mean().clamp_min(1e-6)
        H[k] += d * torch.eye(H[k].shape[0])
    model.train()
    return H
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], hessians: dict | None = None):
...
def main() -> None:
    ...
    # GPTQ calibration: collect Hessians on EMA model from training data (inside training budget)
    hessians = None
    if args.gptq_calib_batches > 0:
        log0(f"gptq:calibrating batches={args.gptq_calib_batches}")
        t_calib = time.perf_counter()
        hessians = collect_hessians(base_model, train_loader, args, device, grad_accum_steps, args.gptq_calib_batches)
        log0(f"gptq:done layers={len(hessians)} time={1000*(time.perf_counter()-t_calib):.0f}ms")
    torch.cuda.synchronize()
    ...
    if args.slot_enabled:
        ...
        torch.cuda.synchronize()
        log0(f"final_slot val_loss:{slot_loss:.4f} val_bpb:{slot_bpb:.4f} "
             f"steps:{args.slot_steps} lr:{args.slot_lr} time:{1000*(time.perf_counter()-t_slot):.0f}ms")
